...
//		Scanner sc = new Scanner(System.in);
	...
	private static int bfs() {
		...
		while(!queue.isEmpty()) {
			...
			// 위
			nx = cur[0] - 1; 
			if(nx>=0 && !visited[nx]) { // 위로 가는 경우이기 때문에 nx<=100000 은 불필요
				// 
				visited[nx] = true;
				path[nx] = cur[0];
			...
			// 아래 
			nx = cur[0] + 1; 
			if(nx<=100000 && !visited[nx]) { // 아래로 가는 경우이기 때문에 nx>=0 은 불필요 
				visited[nx] = true;
				path[nx] = cur[0];
				queue.offer(new int[] {nx, cur[1]+1});
			}
			...
			// 순간이동 하는 경우
			nx = cur[0] << 1; 
			if(nx<=100000 && !visited[nx]) { // 순간이동하는 경우이기 때문에 nx>=0 은 불필요 
				visited[nx] = true;
				path[nx] = cur[0];
				queue.offer(new int[] {nx, cur[1]+1});
			}
